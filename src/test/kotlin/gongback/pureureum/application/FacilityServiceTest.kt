...
class FacilityServiceTest : BehaviorSpec({
    ...
    val facilityService = FacilityService(facilityRepository, userRepository, fileService)
    ...
    Given("사용자 이메일과 시설 정보") {
        ...
        When("원본 파일 이름이 비어있다면") {
            ...
            every { userRepository.getUserByEmail(any()) } returns user
            every { fileService.validateFileName(any()) } throws IllegalArgumentException("원본 파일 이름이 비어있습니다")
        ...
        When("파일 형식이 유효하지 않다면") {
            ...
            every { fileService.getImageType(any()) } throws IllegalArgumentException("파일 형식이 유효하지 않습니다")
        ...
        When("원본 파일 이름이 존재하면서 유효한 파일이라면") {
            ...
            every { userRepository.getUserByEmail(any()) } returns user
            every { fileService.uploadFile(any(), any(), any()) } returns fileKey
            every { fileService.validateFileName(any()) } returns CERTIFICATION_DOC_NAME
