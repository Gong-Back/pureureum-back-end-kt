...
class UserServiceTest : BehaviorSpec({
    ...
    val userService = UserService(fileService, userRepository, smsLogRepository)
    ...
    Given("사용자와 프로필 이미지 정보") {
        ...
        When("원본 파일 이름이 존재하지 않는다면") {
            ...
            every { fileService.validateFileName(file) } throws IllegalArgumentException("원본 파일 이름이 존재하지 않습니다")
        ...
        When("원본 파일 이름이 비어있다면") {
            ...
            every { fileService.validateFileName(file) } throws IllegalArgumentException("원본 파일 이름이 비어있습니다")
            Then("예외가 발생한다.") {
                shouldThrow<IllegalArgumentException> { userService.updatedProfile(user.email, file) }
            }
        }
        ...
        When("파일 형식이 비어있다면") {
            ...
            every { fileService.validateFileName(file) } returns file.name
            every { fileService.getImageType(file) } throws IllegalArgumentException("파일 형식이 유효하지 않습니다")
            Then("예외가 발생한다.") {
                shouldThrow<IllegalArgumentException> { userService.updatedProfile(user.email, file) }
            }
        }
        ...
        When("이미지 형식의 파일이 아니라면") {
            ...
            every { fileService.validateFileName(file) } returns file.name
            every { fileService.getImageType(file) } throws IllegalArgumentException("이미지 형식의 파일만 가능합니다")
        ...
        When("사용자의 기존 프로필 이미지가 별도로 설정한 파일이라면") {
            ...
            every { userRepository.getUserByEmail(any()) } returns user
            every { fileService.validateFileName(file) } returns fileDto.fileKey
            every { fileService.getImageType(file) } returns file.contentType!!
            every { fileService.deleteFile(any()) } just runs
            every { fileService.uploadFile(any(), any(), any()) } returns fileDto.fileKey
